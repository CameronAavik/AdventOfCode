...
public class Day04 : ISolver
{
    public static void Solve(ReadOnlySpan<byte> input, Solution solution)
    {
        ...
        var lines = input.Length / (lineLength + 1);
        ...
        while (col < lineLength)
        {
            ...
            var finalMask64 = finalMask | ((ulong)finalMask << 32);

            for (var row = 0; row < lines; row++)
            {
                var bytes = Vector256.LoadUnsafe(ref inputRef, (nuint)(offset + row * (lineLength + 1)));
                var x = (ulong)Vector256.Equals(bytes, Vector256.Create((byte)'X')).ExtractMostSignificantBits();
                var m = (ulong)Vector256.Equals(bytes, Vector256.Create((byte)'M')).ExtractMostSignificantBits();
                var a = (ulong)Vector256.Equals(bytes, Vector256.Create((byte)'A')).ExtractMostSignificantBits();
                var s = (ulong)Vector256.Equals(bytes, Vector256.Create((byte)'S')).ExtractMostSignificantBits();

                var xmas_h = x & (m >> 1) & (a >> 2) & (s >> 3) & finalMask64;
                part1 += BitOperations.PopCount(xmas_h);

                var samx_h = s & (a >> 1) & (m >> 2) & (x >> 3) & finalMask64;
                part1 += BitOperations.PopCount(samx_h);

                var s_x = (s | (x << 32)) & finalMask64;

                var xmas_samx_v = XMA_SAM_V & s_x;
                part1 += BitOperations.PopCount(xmas_samx_v);

                var xmas_samx_dr = ((XMA_SAM_DR & ShiftLeftMask) << 1) & s_x;
                part1 += BitOperations.PopCount(xmas_samx_dr);

                var xmas_samx_dl = ((XMA_SAM_DL & ShiftRightMask) >> 1) & s_x;
                part1 += BitOperations.PopCount(xmas_samx_dl);

                var s_m = s | (m << 32);
                var mas_sam_dr = (((MA_SA_DR & ShiftLeftMask) << 1) & s_m & ShiftRightMask) >> 1;
                var mas_sam_dl = (((MA_SA_DL & ShiftRightMask) >> 1) & s_m & ShiftLeftMask) << 1;
